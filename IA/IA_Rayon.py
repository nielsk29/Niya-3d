import IA_globalVariable as glb  # importation des Variables globals
import time
import IA_Draw3d
import IA_objet
import math
...
def rays(murBrique):   # fonction qui envoie les rayons
    ...
    listAngleObj = IA_objet.CreerListeAngleOBJ(glb.listeObjet)
    pente2 = -math.tan(angleRay)

    for i in range(glb.nbRay):  # boucle de chaque rayon

        distanceRay=1  # variable qui contient la distance entre le joueur et la position du rayon
        ...
        for nbCar in range(glb.diagCarre):  # boucle qui va envoyé le rayon à la prochaine frontière entre les carrés donc le nombre maximal de cette boucle est le diagonal de la carte
            ...
            glb.listeRond.append(((int(posRayX * glb.minimap[0] / glb.gameX), int(posRayY * glb.minimap[1] / glb.gameY)),(0,255,0)))  # ajoute un rond à la frontière pour la miniMap
            if glb.Carte[carre] == "1":  # si le rayon touche un mur
                glb.listeRay.append(((glb.mapPlayerX, glb.mapPlayerY),(posRayX * glb.minimap[0] / glb.gameX, posRayY * glb.minimap[1]  / glb.gameY)))  # ajoute le rayon pour la miniMap
                #objet.SaveObjet(i, pente, distanceRay, cosAngle, sinAngle)  # utilise la fonction qui regarde si le rayon à toucher un objet
                pente2 = pente
                IA_Draw3d.rect3d(i,angleRay,distanceRay,cote,posRayX,posRayY,murBrique,colPosRay,lignePosRay)  # Utilise la fonction qui va dessiner le rectangle (avec un bout de l'image de mur) correspondant au rayon
                IA_objet.OBJ(i, distanceRay, angleRay, listAngleObj, pente, posRayX, posRayY)
                break  # stop la boucle, car le rayon a touché un mur
            if fin:
                glb.listeRay.append(((glb.mapPlayerX, glb.mapPlayerY),(posRayX * glb.minimap[0]  / glb.gameX, posRayY * glb.minimap[1]  / glb.gameY)))
                IA_objet.OBJ(i, distanceRay, angleRay, listAngleObj, pente, posRayX, posRayY)
                break  # stop la boucle, car le rayon a touché un mur

            if glb.Carte[carre] == "2" and glb.statusPorte[carre][0] == 0:
                    fin = True
    IA_objet.drawObjet2d()
    ...
    for nb in range(glb.diagonal):  # boucle qui va calculer le prochain pixel donc pour maximum de répétition le diagonal des pixels
        ...
        if glb.Carte[carre] == "1":  # si le rayon touche un mur
            if (posRayX%glb.rectSizeX)<1 or 99<(posRayX%glb.rectSizeX):  # on regarde si on a touché un mur perpendiculaire à l'axe X
                ...
                cote=2
            IA_Draw3d.rect3d(i, angleRay, nb, cote, posRayX, posRayY, murBrique,colPosRay,lignePosRay)  # Utilise la fonction qui va dessiner le rectangle (avec un bout de l'IA_image de mur) correspondant au IA_Rayo
            IA_objet.OBJ(i, nb, angleRay, listAngleObj, pente, posRayX, posRayY)
            break  # stop la boucle, car le rayon a touché un mur
