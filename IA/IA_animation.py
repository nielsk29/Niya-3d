...
def anim():
    #glb.statusMonstre[1][8] = (glb.statusMonstre[1][8] + 0.1) % glb.pi2
    glb.a_port = (glb.a_port + 0.03) % glb.pi2
    ...
    for element in glb.listeBall:  # on parcourt la liste des rockets
        ...
        anglePlayer = IA_objet.calculAngleObj(element[1]-glb.playerX, element[2]-glb.playerY)  # angle du joueur vers la rocket
        ...
        if glb.vieMonstre[nb]>0 :  # si il est vivant
            ...
            if glb.statusMonstre[nb][1]:  # s'il est entrain de tiré
                ...
                if frame >=13:  # si on est à la dernière frame
                    ...
                    angle = IA_objet.calculAngleObj((glb.playerX - glb.listeMonstre[nb][1]),
            ...
            """if element[5] :
                ...
                angle = IA_objet.calculAngleObj(glb.listeMonstre[nb][1]-pointAriver[0], glb.listeMonstre[nb][2]-pointAriver[1])
        ...
        for event in pygame.event.get():  # recupère tout les évenement
            if event.type == pygame.MOUSEBUTTONDOWN: # si on a cliqué
                ...
                if restart.collidepoint(x, y):  # si on cliqué sur restart
                    IA_chargement.restart()  # on remet à 0 toute les variables
                    nbTerminer = [False] * len(listeRectNoir)  # on met que tout les rectangle on pas fini leur animation
                    while verif :
                        IA_image.f_all(glb.murBrique)  # affiche l'image 3d
                        IA_menu.afficher()  # affiche le HUD
