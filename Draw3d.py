...
def rect3d (iray,rayAngle,nb,cote,rayX,rayY,imageMur):  # fonction qui dessine un rectangle correspondant à un rayon
    ...
    RectY = math.ceil((glb.screenY-imageSizeY)/2)  # calcul posY du rectangle
    ...
    if (Ximage+glb.RectLarg)>imageSizeX:  # teste si on arrive trops loin sur l'image donc qu'on veut prendre une partie qui n'existe par sur l'image
        mur = mur.subsurface((0, 0, glb.RectLarg, imageSizeY))  # coupe l'image pour avoir que la dernière partie possible de la taille d'un rectangle dans l'image
    else:
    ...
    debut = time.time()
    if imageSizeY < (glb.screenY/1.5):
        glb.screen.blit(glb.rectSombre[imageSizeY][0], (iray*glb.RectLarg, RectY))
